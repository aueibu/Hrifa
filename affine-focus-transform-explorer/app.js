(() => {
  ...
  function renderGraph() {
    ...
    const p=params(), multiplierColors=["var(--transform-edge)","var(--derived)","var(--status-ok)","var(--seed)","var(--highlight)","var(--variant)"];
    state.lastLayout=state.layout;
    const layout=positions(c,state.layout), pos=layout.points, w=layout.width, h=layout.height;
    let svg=`<svg class="graph-svg" width="${w}" height="${h}" viewBox="0 0 ${w} ${h}" role="img"><defs><marker id="arrow" markerWidth="5" markerHeight="5" refX="4.5" refY="2.5" orient="auto"><path d="M0,0 L5,2.5 L0,5 z" style="fill:var(--transform-edge)"/></marker></defs><g class="graph-viewport" transform="translate(${state.panX} ${state.panY}) scale(${state.zoom})">`;
    if(state.layout==="scc"){
      const palette=["var(--seed)","var(--derived)","var(--status-ok)","var(--highlight)","var(--transform-edge)","var(--variant)"];
