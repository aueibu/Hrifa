...
| --- | --- | --- | --- | --- | --- |
| Surface | Shared | `--neutral-surface-bg` | `gray.50` `#F6EFE7` | `gray.900` `#1E1A1A` | Default page/canvas background. |
| Surface, raised | Shared | `--neutral-surface-raised` | `gray.100` `#ECE5E0` | `gray.950` `#141111` | Panels and instrument chrome. |
| Surface, overlay | Shared | `--neutral-surface-overlay` | `gray.200` `#CEC8C5` | `gray.800` `#383736` | Modals, dropdowns, popovers. |
| Border | Shared | `--neutral-border` | `gray.300` `#BBB6B0` | `gray.700` `#52514F` | Default rules and dividers. |
| Border, strong | Shared | `--neutral-border-strong` | `gray.400` `#A19D97` | `gray.600` `#696663` | Emphasized rules, active containers. |
| Text | Shared | `--neutral-text` | `gray.950` `#141111` | `gray.50` `#F6EFE7` | Default ink. |
| Text, secondary | Shared | `--neutral-text-secondary` | `gray.900` `#1E1A1A` | `gray.200` `#CEC8C5` | Secondary or supporting text — labels, captions, instructional copy. Legible at a glance, subordinate to default ink. |
| Text, muted | Shared | `--neutral-text-muted` | `gray.700` `#52514F` | `gray.400` `#A19D97` | Tertiary or de-emphasized text — fine print, nested meta, quiet instrument chrome. |
| Text, disabled | Shared | `--neutral-text-disabled` | `gray.400` `#A19D97` | `gray.600` `#696663` | Disabled labels and values. |
| Shadow | Shared | `--neutral-shadow` | `gray.800` `#383736` | `gray.300` `#BBB6B0` | Drop-shadow / offset-accent ink (button hover offsets, panel shadow). |
| Chrome accent | Chrome | `--chrome-accent` | `blue.700` `#2C5474` | `blue.600` `#3D6A92` | Buttons, sliders, links, focus rings, dropdown hover/inset accent. Same hue in both themes — dark mode uses one step lighter rather than switching hue. |
| Chrome accent, secondary | Chrome | `--chrome-accent-secondary` | `green.700` `#325943` | `green.600` `#2D734F` | A second chrome hue for an action that needs visual weight without being the primary CTA. Same shade-band convention and luminance as `--chrome-accent`, so it shares `--chrome-ink`. Not read by work-surface code, despite sharing green's hue family with `--work-surface-idle`. |
| Chrome accent, tertiary | Chrome | `--chrome-accent-tertiary` | `purple.700` `#59487B` | `purple.600` `#715B9E` | A third chrome hue, same rationale as secondary. Known tuning candidate: shares its light-mode step with `--work-surface-relation`. |
| Chrome ink | Chrome | `--chrome-ink` | `gray.50` `#F6EFE7` | `gray.50` `#F6EFE7` | Text/icon colour for anything sitting on a `--chrome-accent`/`-secondary`/`-tertiary` fill. Fixed, not theme-inverted — see [Fill tokens need an ink pair](../design-tokens/README.md#fill-tokens-need-an-ink-pair). |
| Chrome accent, text | Chrome | `--chrome-accent-text` | `blue.700` `#2C5474` | `blue.400` `#68A2D7` | `--chrome-accent` used AS running text directly on a neutral surface (outline-button labels, links, eyebrow labels) instead of as a fill. `--chrome-accent`'s own dark value is ~3.0:1 as text — fails WCAG AA; this is ~6.4:1. Borders/focus rings stay on `--chrome-accent` itself (non-text 3:1 minimum, which it already clears) — only `color` moves to this token. See [the reverse case](../design-tokens/README.md#the-reverse-case-a-fill-token-used-as-text). |
| Chrome accent, secondary text | Chrome | `--chrome-accent-secondary-text` | `green.700` `#325943` | `green.400` `#57AE7F` | `--chrome-accent-secondary`'s text counterpart — see chrome accent, text. |
| Chrome accent, tertiary text | Chrome | `--chrome-accent-tertiary-text` | `purple.700` `#59487B` | `purple.400` `#A892D7` | `--chrome-accent-tertiary`'s text counterpart — see chrome accent, text. |
| Chrome accent, soft | Chrome | `--chrome-accent-hover` (app-local, per applet) | derived | derived | Hover fill behind ghost buttons and menu items — a `color-mix()` of `--chrome-accent`, not a token; see [Design tokens](#design-tokens). Generalized to any `--btn-color` in `components.css`. |
| Work-surface idle | Work surface | `--work-surface-idle` | `green.800` `#253C2E` | `green.300` `#7BC69B` | Available but unselected points, edges, or objects. |
| Work-surface hover | Work surface | `--work-surface-hover` | `orange.500` `#D95D35` | `orange.200` `#F3BF90` | Hovered point/edge/object, prior to selection. |
| Work-surface active | Work surface | `--work-surface-active` | `orange.600` `#935832` | `orange.300` `#F1A57B` | Selected, current, or provisional/preview geometry. Deliberately shares hover's hue — the two states are told apart by size/weight/dash treatment, not colour. When both a hover and an active/preview cue can appear on the same element, keep that non-colour distinction unambiguous. |
| Work-surface relation | Work surface | `--work-surface-relation` | `purple.700` `#59487B` | `purple.300` `#C0ADE6` | A second object referenced against the active one — comparison, not selection. |
| Emphasis | Shared, rare | `--emphasis` | `yellow.500` `#9B803F` | `yellow.300` `#D5B442` | Rare warm or editorial highlight. Never the default selection colour. |
| Critical | Shared | `--critical` | `red.500` `#E5504B` | `red.500` `#E5504B` | Destructive, critical, or irreversible action. Fixed, not theme-inverted, so it reads as consistently serious rather than blending into either theme's palette. |
| Critical, text | Shared | `--critical-text` | `red.700` `#823B3B` | `red.400` `#D88885` | `--critical` used AS running text directly on a neutral surface (danger-outline labels, error copy) instead of as a fill. `--critical`'s one fixed value happens to pass as text in dark theme (~4.6:1) but fails in light (~3.3:1) — the mirror image of chrome's problem, same root cause. Borders/focus rings stay on `--critical` itself — only `color` moves to this token. |
| Critical ink | Shared | `--critical-ink` | `gray.950` `#141111` | `gray.950` `#141111` | Text/icon colour for anything sitting on a `--critical` fill (danger-button hover). Fixed for the same reason as `--chrome-ink`. |
...
### Fill tokens carry their own ink

`--neutral-text` and `--neutral-surface-bg` invert cleanly between themes by construction — each theme's ink sits at the opposite end of the ramp from that theme's own surface. Accent-ish fills (`--chrome-accent`, `--critical`) do not follow that rule: their whole point is to stay recognizable as "the accent" or "the danger colour" across both themes, so they don't invert, and sometimes barely change tone between themes at all. That means text sitting on top of one of them must never borrow `--neutral-surface-bg` or `--neutral-text` for contrast — those tokens only happen to work in whichever theme the fill's luminance happens to favour, and silently break in the other theme, or break outright the next time the fill is retuned. Every fill role gets its own fixed `-ink` companion instead — `--chrome-ink`, `--critical-ink` — chosen once for contrast against that specific fill. See [design-tokens' "Fill tokens need an ink pair"](../design-tokens/README.md#fill-tokens-need-an-ink-pair) for the mechanics.
...
3. **Semantic** — the only layer components and canvas-drawing code should reference: `--neutral-surface-bg`, `--neutral-surface-raised`, `--neutral-surface-overlay`, `--neutral-border`, `--neutral-border-strong`, `--neutral-text`, `--neutral-text-secondary`, `--neutral-text-muted`, `--neutral-text-disabled`, `--neutral-shadow`, `--chrome-accent`, `--chrome-ink`, `--work-surface-idle`, `--work-surface-hover`, `--work-surface-active`, `--work-surface-relation`, `--emphasis`, `--critical`, `--critical-ink` — built from `design-tokens/color-tokens.json`'s `color.theme.light.*` / `color.theme.dark.*`, scoped by a `[data-theme]` selector so light and dark share the same variable names. Where canvas code reads computed styles into a JS object (as Edel's `uiTheme` does), that object's keys should carry the same semantic sense, not the primitive hue names — so `uiTheme.active`/`uiTheme.hover`/`uiTheme.idle`/`uiTheme.relation` stay legible independent of which hue currently backs them.
